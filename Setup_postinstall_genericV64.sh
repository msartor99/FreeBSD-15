...
# Version: 6.4 - Pure POSIX /bin/sh | Multi-Desktop | Idempotent
# ==============================================================================
...
# Installation de bsddialog si absent
...
# 3. CHOIX DES BUREAUX
DESKTOPS_RAW=$(bsddialog --title "Environnements de bureau" \
    --checklist "Sélectionnez vos bureaux (Espace pour cocher) :" 15 60 5 \
    "PLASMA6" "KDE Plasma 6" off \
    "GNOME"   "GNOME Desktop" off \
    "XFCE4"   "XFCE 4" off \
    "MATE"    "MATE Desktop" off \
    "LXQT"    "LXQt" off 3>&1 1>&2 2>&3)

# Nettoyage sécurisé pour sh (pas de tableaux bash)
DESKTOPS=$(echo "$DESKTOPS_RAW" | tr -d '"')

# 4. GESTION RÉSEAU (Correction POSIX)
# On construit la liste sans eval pour éviter les erreurs de parsing
INTERFACES=$(ifconfig -l | tr ' ' '\n' | grep -v 'lo0')
IF_LIST=""
for iface in $INTERFACES; do
    IF_LIST="$IF_LIST $iface [Detectee]"
done

CHOIX_IF=$(bsddialog --title "Configuration Réseau" \
    --menu "Choisir l'interface par défaut :" 15 70 6 \
    $IF_LIST "WIFI" "[Config_WLAN]" 3>&1 1>&2 2>&3)
...
pkg install -y xorg seatd sddm firefox vlc ffmpeg webfonts dejavu noto-basic liberation-fonts-ttf fusefs-exfat fusefs-ntfs realtek-re-kmod
...
case "$CHOIX_IF" in
    "WIFI") 
        W_DEV=$(sysctl -n net.wlan.devices | awk '{print $1}')
        if [ -n "$W_DEV" ]; then
            sysrc wlans_"$W_DEV"="wlan0"
...
case "$CHOIX_CPU" in
    "AMD")   
        ...
        grep -q "amd-ucode" /boot/loader.conf || echo 'cpu_microcode_name="/boot/firmware/amd-ucode.bin"' >> /boot/loader.conf 
        ;;
    "INTEL") 
        pkg install -y cpu-microcode-intel
        sysrc -f /boot/loader.conf coretemp_load="YES"
        grep -q "intel-ucode" /boot/loader.conf || echo 'cpu_microcode_name="/boot/firmware/intel-ucode.bin"' >> /boot/loader.conf 
        ;;
esac
...
if ! grep -q "$CLASS" /etc/login.conf; then
    printf "$CLASS|Custom User Class:\\\n\t:charset=UTF-8:\\\n\t:lang=${L_CODE}.UTF-8:\\\n\t:tc=default:\n" >> /etc/login.conf
    cap_mkdb /etc/login.conf
fi

# 9. CLAVIER X11 (POSIX Cat)
...
# 10. AUDIO & BUREAUTIQUE
pkg install -y pipewire pavucontrol libreoffice cups hplip nss_mdns samba416 virtualbox-ose-72
sysrc cupsd_enable="YES" samba_server_enable="YES"

# 11. INSTALLATION DES BUREAUX (Boucle compatible sh)
...
# 12. FINALISATION MDNS & VBOX
if ! grep -E "^hosts:.*mdns" /etc/nsswitch.conf >/dev/null 2>&1; then 
...
bsddialog --msgbox "Post-installation V6.4 (POSIX) terminee.\nRedemarrez pour appliquer." 10 50